...
Sistemul este organizat pe trei niveluri ierarhice, prezentate în Figura~\ref{fig:class-deps}. La bază se află operațiile pe biți, urmate de encoderele și decoderele specializate, iar la vârf se află modulele de stocare.
...
\begin{tikzpicture}[
    ...
    % Nivelul 3: Stocare
    \node[module] (mvb) at (0,4) {MultiVariateBlock};
    \node[module] (mvd) at (6.5,4) {MultiVariateDecoder};

    % Nivelul 4
    \node[module] (mvs) at (3.25,5.5) {MultiVariateSeries};

    % Nivelul 5
    \node[module] (store) at (3.25,7) {MultiVariateStore};
    ...
    \node[gray, anchor=east] at (-3,4.75) {\footnotesize Nivel 3: Stocare};
\end{tikzpicture}
